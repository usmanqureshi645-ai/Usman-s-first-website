...
// LLMs frequently "tidy up" punctuation when quoting (curly vs straight quotes/apostrophes,
// en/em dashes) even when told to be verbatim. These substitutions are all 1-for-1 character
// swaps, so the resulting index is still valid against the original (un-normalized) string.
...
function findQuoteIndex(decoded, quote) {
  const normalizedHaystack = normalizeForMatch(decoded);
...
function injectCommentIntoDocumentXml(documentXml, quote, commentId) {
  // Prefer matching a whole <w:r>[<w:rPr>...</w:rPr>]<w:t>...</w:t></w:r> run so the
  // original run formatting (rPr — bold, font, etc.) can be preserved on every split piece.
  const rRegex = /<w:r(?:\s[^>]*)?>(?:(<w:rPr>[\s\S]*?<\/w:rPr>))?<w:t([^>]*)>([^<]*)<\/w:t><\/w:r>/g;
  let match;
  while ((match = rRegex.exec(documentXml)) !== null) {
    const [fullMatch, rPr, attrs, rawText] = match;
    const decoded = decodeXmlText(rawText);
    const idx = findQuoteIndex(decoded, quote);
    if (idx === -1) continue;
    ...
    const cleanAttrs = attrs.replace(/\s*xml:space="preserve"/, '');
    const rPrTag = rPr || '';

    const run = (content) => `<w:r>${rPrTag}<w:t${cleanAttrs} xml:space="preserve">${content}</w:t></w:r>`;
