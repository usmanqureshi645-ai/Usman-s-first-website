...
export async function logAndCheckUsage(req, { kvUrl, kvToken }, tool) {
  if (!kvUrl || !kvToken) return { count: 0, shouldNudge: false, limited: false };

  const ip = getClientIp(req);
  if (ip === 'unknown') return { count: 0, shouldNudge: false, limited: false };

  const limited = await isRateLimited(kvUrl, kvToken, ip);
  if (limited) return { count: 0, shouldNudge: false, limited: true };
  ...
  try {
    ...
    return { count: record.count, shouldNudge: record.count >= 2, limited: false };
  } catch {
    return { count: 0, shouldNudge: false, limited: false };
  }
}
