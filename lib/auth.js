...
// Email(s) allowed to reach the admin dashboard via ordinary site login, no key needed.
const ADMIN_EMAILS = ['muhammadbdo370@gmail.com'];

// Admin gate used by every dashboard-* action: pass either the DASHBOARD_KEY (query/header,
// still supported for scripts/cron) or be logged in (site_session cookie) as an ADMIN_EMAILS address.
export function isAdminRequest(req, providedKey) {
  if (verifyAdminKey(providedKey)) return true;
  const user = getUserFromRequest(req);
  return !!user && ADMIN_EMAILS.includes(normalizeEmail(user.email));
}
