...
const PERSONA_VOICES = {
  ...
  'Amelia Hartley':     { voice: 'Kendra',   language: 'en-US' }, // US female
  'Marcus Lee':         { voice: 'Arthur',   language: 'en-GB' }, // UK male
  'Elena Rossi':        { voice: 'Olivia',   language: 'en-AU' }, // Australian female
  'James Carter':       { voice: 'Joey',     language: 'en-US' }, // US male
  'Charlotte Sinclair': { voice: 'Joanna',   language: 'en-US' }, // US female
  'Eleanor Hughes':     { voice: 'Salli',    language: 'en-US' }, // US female
  'Quiz Coach':         { voice: 'Kevin',    language: 'en-US' }, // US male
  'default':            { voice: 'Matthew',  language: 'en-US' }, // US male (GAAP Champion)
};
...
const PERSONA_VOICE_SETTINGS = {
  ...
  'David Whitfield':    { rate: '1.0',  loudness: '+8dB' },  // deliberate, authoritative
  'Amelia Hartley':     { rate: '1.12', loudness: '+9dB' },  // upbeat, fast-paced
  'Marcus Lee':         { rate: '1.08', loudness: '+8dB' },  // confident, direct
  'Elena Rossi':        { rate: '1.15', loudness: '+9dB' },  // most energetic
  'James Carter':       { rate: '1.02', loudness: '+8dB' },  // composed, grounded
  'Charlotte Sinclair': { rate: '1.1',  loudness: '+9dB' },  // enthusiastic
  'Eleanor Hughes':     { rate: '1.07', loudness: '+8dB' },  // warm, encouraging
...
// Voices that support Polly's <amazon:emotion> SSML tag (Neural engine only).
// Applying this tag to unsupported voices causes synthesis to silently fail and
// fall back to browser TTS — so this list must only contain confirmed supported voices.
const EMOTION_CAPABLE_VOICES = new Set([
  'Joanna', 'Matthew', 'Kendra', 'Kimberly', 'Salli', 'Ivy', 'Kevin', 'Joey',
]);

// Voices that support <amazon:domain name="conversational"> for a more casual,
// less newscaster delivery. Only Joanna and Matthew confirmed on neural engine.
const CONVERSATIONAL_VOICES = new Set(['Joanna', 'Matthew']);
...
function wrapConversational(text, voiceId) {
  if (!CONVERSATIONAL_VOICES.has(voiceId)) return text;
  return `<amazon:domain name="conversational">${text}</amazon:domain>`;
}
...
async function synthesizeWithPolly(text, personaName, pollyClient) {
  ...
  const withDomain = wrapConversational(withEmotion, config.voice);
  const ssmlText = `<speak><prosody rate="${settings.rate}" volume="${settings.loudness}">${withDomain}</prosody></speak>`;
