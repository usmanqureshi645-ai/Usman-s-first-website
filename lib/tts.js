...
async function synthesizeWithPolly(text, personaName, pollyClient) {
  ...
  if (!cleanedText || cleanedText.length === 0 || cleanedText.length > 2500) {
    console.error(`[TTS] Text validation failed for ${personaName}: length=${cleanedText?.length || 0}`);
    return null;
  }
  ...
  try {
    console.log(`[TTS] Creating SynthesizeSpeechCommand for ${config.voice} (${personaName})`);
    ...
    console.log(`[TTS] Sending command to Polly...`);
    const response = await pollyClient.send(command);

    if (!response.AudioStream) {
      console.error(`[TTS] Polly returned empty AudioStream for ${personaName}`);
      return null;
    }

    console.log(`[TTS] Got AudioStream, converting to bytes...`);
    const audioBuffer = await response.AudioStream.transformToByteArray?.();
    if (!audioBuffer) {
      console.error(`[TTS] transformToByteArray failed or returned null for ${personaName}`);
      return null;
    }

    console.log(`[TTS] Converting ${audioBuffer.length} bytes to base64...`);
    const base64Audio = Buffer.from(audioBuffer).toString('base64');
    console.log(`[TTS] Successfully synthesized audio for ${personaName} (${base64Audio.length} chars base64)`);
