...
// commands: array of arg-arrays, e.g. [['INCR','k'], ['GET','k2']].
async function pipe({ kvUrl, kvToken }, commands) {
...
export async function recordSignup(kv, { name, email }) {
  try {
    const entry = JSON.stringify({ name, email, createdAt: new Date().toISOString() });
    await pipe(kv, [
      ['INCR', 'metrics:signups:total'],
      ['LPUSH', 'signups_log', entry],
      ['LTRIM', 'signups_log', '0', '499'],
    ]);
  } catch { /* non-fatal */ }
}
