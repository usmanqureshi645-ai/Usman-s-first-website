...
export function buildMeetingSystemPrompt(agents) {
  const activeAgents = Array.isArray(agents) && agents.length ? agents.filter(a => ALL_PERSONAS[a]) : Object.keys(ALL_PERSONAS);
  const rosterText = activeAgents.map(name => `- ${name} — ${ALL_PERSONAS[name]}`).join('\n');
